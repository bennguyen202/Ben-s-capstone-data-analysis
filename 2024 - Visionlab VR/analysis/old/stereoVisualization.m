...
% Configuration
sub = '004';
experiment_type = 'acuity'; % Change to 'contrast' for contrast sensitivity
eccentricity = 5; % Only used for acuity
test_number = '02'; % Specify the test number
...
for j = 1:length(eye_conditions)
    ...
    if strcmp(experiment_type, 'acuity')
        ...
        values = arrayfun(@(entry) abs(entry.LogLum), eye_data);  % Add abs() here
    end
...
elseif strcmp(experiment_type, 'contrast')
    ylim([0, 2]);  % Changed from [-2, 0] to [0, 2]
end
...
function plot_data_with_line(ax, trial_numbers, values, color, index, ecc_number, eye_condition, experiment_type)
    ...
    if strcmp(experiment_type, 'acuity')
        ...
        annotation('textbox', [0.8, 0.85 - 0.05 * index, 0.1, 0.1], 'String', sprintf('Threshold (%s): %.2f Log CS', eye_condition, avg_final_values), ...  % Changed LogLum to Log CS
            'EdgeColor', 'none', 'Color', color, 'FontSize', fontSize, 'HorizontalAlignment', 'right');
        ylabel(ax, 'Log Contrast Sensitivity');  % Updated ylabel
    end
