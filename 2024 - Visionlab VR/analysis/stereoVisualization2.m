...
% Configuration
sub = '002';
experiment_type = 'contrast'; %'contrast'; % 'acuity' or 'contrast'
eccentricity = 5; % Only used for acuity
test_number = '03'; % Specify the test number
...
for j = 1:length(eye_conditions)
    ...
    plot_data_with_line(ax, trial_numbers, values, colors(eye_conditions{j}), j, eccentricity, eye_conditions{j}, experiment_type);
end
